...
public class RustRootModule : RustModule
{
    ...
    public override void Export(StringBuilder builder, int indentLevel)
    {
        ...
        builder.AppendLine("#![allow(clippy::all)]");
        builder.AppendLine("use crate::internal::*;");
        builder.AppendLine("use ffxiv_client_structs_macros::*;");
        builder.AppendLine(
            "use ffxiv_client_structs_util::*;");
        builder.AppendLine("use ffxiv_client_structs_util::address::*;");
        builder.AppendLine("use ffxiv_client_structs_util::macros::*;");
    ...
    private void ExportVTableResolver(StringBuilder builder, string indent, bool async)
    {
        ...
        if (async)
        {
            ...
            builder.AppendLine("pub async unsafe fn resolve_vtables_async(resolver: VTableResolver) {");
            builder.AppendLine($"{indent}let mut tasks = tokio::task::JoinSet::new();");
            builder.AppendLine($"{indent}let mut waiting = 0;");
        }
        else
        {
            builder.AppendLine("pub unsafe fn resolve_vtables(resolver: VTableResolver) {");
        }

        var prefix = async ? "waiting += 1; tasks.spawn(" : "";
        var suffix = async ? ");" : ";";
        ...
        if (async)
        {
            builder.AppendLine($"{indent}while waiting > 0 {{");
            builder.AppendLine($"{indent}    if let Some(_) = tasks.join_next().await {{");
            builder.AppendLine($"{indent}        waiting -= 1;");
    ...
    private void ExportStaticAddressResolver(StringBuilder builder, string indent, bool async)
    {
        ...
        if (async)
        {
            builder.AppendLine("#[cfg(feature = \"async-resolution\")]");
            builder.AppendLine(
                "pub async unsafe fn resolve_static_addresses_async(resolver: StaticAddressResolver) {");
            builder.AppendLine($"{indent}let mut tasks = tokio::task::JoinSet::new();");
            builder.AppendLine($"{indent}let mut waiting = 0;");
        }
        else
        {
            builder.AppendLine(
                "pub unsafe fn resolve_static_addresses(resolver: StaticAddressResolver) {");
        }
    ...
    private void ExportMemberFunctionResolver(StringBuilder builder, string indent, bool async)
    {
        ...
        if (async)
        {
            builder.AppendLine("#[cfg(feature = \"async-resolution\")]");
            builder.AppendLine(
                "pub async unsafe fn resolve_member_functions_async(resolver: MemberFunctionResolver) {");
            builder.AppendLine($"{indent}let mut tasks = tokio::task::JoinSet::new();");
            builder.AppendLine($"{indent}let mut waiting = 0;");
        }
        else
        {
            builder.AppendLine(
                "pub unsafe fn resolve_member_functions(resolver: MemberFunctionResolver) {");
        }
